...
<body>
	<header>
		...
		<div class="jumbotron fade-in">
			<div class="kotak2 d-flex justify-content-center align-items-center">
				<div>
					<h5 class="text-center">Objek Wisata di</h5>
	...
	<main class="content">
		<h3 class="mt-5 mb-5 text-center fade-in-heading">Tentang Wisata</h3>
		<audio id="myAudio" loop>
			<source src="{{ asset('umbul-umbulblambangan.mp3') }}" type="audio/mpeg">
			Your browser does not support the audio element.
		</audio>
			@foreach($objekwisata as $key => $item)
			<section class="about pos mt-5 slide-in">
				<div class="container">
					<div class="row">
						@if($key % 2 === 0)
						<div class="col-lg-6">
							<img src="{{ asset('/gambar_objekwisata/' . $item->gambar_objekwisata) }}" alt="">
						</div>
						<div class="col-lg-6 d-flex align-items-center">
							<div class="wrap ms-5">
								<div>
									<h1>{{ $item->judul_objekwisata }}</h1>
								</div>
								<p>
									{{ $item->deskripsi_objekwisata }}
								</p>
							</div>
						</div>
						@else
